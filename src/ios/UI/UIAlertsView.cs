...
{
	public class UIAlertsView : UIView
	{
		...
		private AlertsViewSource ViewSource { get{return this._tableView.Source as AlertsViewSource;}}

		public Action<Alert> ItemClicked {get;set;}

		private UIAlertsTableView _tableView;
			...
			Api.Default.GetAlertsAsync ((results)=>{
				InvokeOnMainThread(()=> {
					if(results.Error == null){
						_tableView.Source = new AlertsViewSource(results.Results.ToList());
						(_tableView.Source as AlertsViewSource).RowClicked += RowClickHandler;
		...
		private void RowClickHandler(object sender, RedBit.RowClickedEventArgs<Alert> e){
			if (ItemClicked != null)
				ItemClicked (e.Item);
		}
	...
	public class UIAlertsTableView : UITableView
	{
		public UIAlertsTableView(){
		}

		public Action<Alert> DeleteItem { get; set; }
	}
